function [PEB,P]   = GNG_spm_dcm_peb(P,M,field)
...
for n = 1:maxit
    ...
    for i = 1:Ns
        ...
        Xi         = kron(X(i,:),W);
        rE{i}      = Xi*b;
        % output of function below is a reduced model (the estimated free
        % energy/posterior had we used the group level priors, rE and rC)
        ...
        for j = 1:Ng
            % change in free energy from full-> reduced; increase in certainty over parameter estimates 
            % i.e. reduction in variance(accuracy)- how much parameters change from prior to posterior (complexity)
            dFdgj      = exp(g(j))*(trace((rC - sC)*Q{j}) - dE'*Q{j}*dE)/2;
            % dFdg(j) stores aggregate 2nd-level change in free energy
            dFdg(j)    = dFdg(j) + dFdgj;
            dFdgg(j,j) = dFdgg(j,j) + dFdgj;
            % gradient of 2nd-level free energy across parameters
            dFdbgj     = exp(g(j))*(Xi - sC*rP*Xi)'*Q{j}*dE;
             % dFdbg(:,j) stores aggregate 2nd-level gradient
            dFdbg(:,j) = dFdbg(:,j) + dFdbgj;
            % curvature/acceleration of 2nd-level free energy
            for k = 1:Ng
    ...
    % remember that 2nd level free energy is 1st-level free energy - second
    % level complexity
    F     = F - Fc;
